...
$enableCiresonIntegration = $false
                ...
                switch ($message.From)
                {
                    $affectedUserSMTP.TargetAddress {
                        ...
                        switch -Regex ($commentToAdd) {
                            ...
                            "\[$removeWatchlistKeyword]" {if ($enableCiresonIntegration -and ($addWatchlistKeyword.Length -gt 1)) {$cpu = Get-CiresonPortalUser -username $commentLeftBy.UserName -domain $commentLeftBy.Domain; if ($cpu) {Remove-CiresonWatchListUser -userguid $cpu.Id -workitemguid $workItem.__InternalId} }}
